...
const loadDuneContract = (instance, constructor, contract) => {
  instance.address = contract.address;

  // User defined methods
  // NOTE: overloaded methods not currently supported in ligo,
  // events not currently supported in Dune (as of writing this comment)
  for (const method in contract.methods) {
    const loadMethod = tezosMethod => {
      let fn;

      fn = execute.sendDune.call(
        constructor,
        tezosMethod,
        method,
        instance.address
      ...
      // NOTE: views not supported in Dune (as of writing this comment)
      /*
      ...
      fn.sendTransaction = execute.sendDune.call(
        constructor,
        tezosMethod,
        method,
        instance.address
      ...
      // NOTE: current request functionality not supported in Dune
  ...
  // NOTE: events not supported in Dune (as of writing this comment)
  // Other events
...
module.exports = loadDuneContract;
