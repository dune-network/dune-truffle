...
```
$ npm install @dune-network/contract
```
...
First, set up a new web3 provider instance and initialize your contract, then `require("@dune-network/contract")`. The input to the `contract` function is a JSON blob defined by [@dune-network/contract-schema](https://github.com/dune-network/truffle/tree/master/packages/contract-schema). This JSON blob is structured in a way that can be passed to all truffle-related projects.
...
In your `head` element, include @dune-network/contract:
...
With this usage, `@dune-network/contract` will be available via the `TruffleContract` object:

```
var MyContract = TruffleContract(...);
```

**Note**: Web3 and its dependencies are now bundled into @dune-network/contract
v4.0.2 or higher.

### Full Example

Let's use `@dune-network/contract` with an example contract from [Dapps For Beginners](https://dappsforbeginners.wordpress.com/tutorials/your-first-dapp/). In this case, the abstraction has been saved to a `.sol` file by [@dune-network/artifactor](https://github.com/dune-network/truffle/tree/master/packages/artifactor):

```javascript
// Require the package that was previosly saved by @dune-network/artifactor
...
// how @dune-network/contract allows for easy control flow.
...
MetaCoin.at(contract_address).then(function(instance) {
  ...
  // This code block will not be executed until @dune-network/contract has verified
  // the transaction has been processed and it is included in a mined block.
  // @dune-network/contract will error if the transaction hasn't been processed in 120 seconds.
...
Even more helpful, however is we *don't even need* to use `.call` when a function is marked as `view` or `pure`, because `@dune-network/contract` will automatically know that that function can only be interacted with via a call:
...
This package is the result of breaking up EtherPudding into multiple modules. Tests currently reside within [@dune-network/artifactor](https://github.com/dune-network/truffle/tree/master/packages/artifactor) but will soon move here.
