...
import { DuneToolkit, Dune } from "@dune-network/taquito";
...
export class DuneAdapter implements InterfaceAdapter {
  public dune: DuneToolkit;
  constructor({ provider }: DuneAdapterOptions) {
    this.dune = Dune;
    if (provider) this.setProvider(provider);
  }

  public async getNetworkId() {
    const { chain_id } = await this.dune.rpc.getBlockHeader();
    return chain_id;
  }
  ...
  public async getBlock(blockNumber: BlockType) {
    ...
    const { hard_gas_limit_per_block } = await this.dune.rpc.getConstants();
    const block = await this.dune.rpc.getBlockHeader({
      block: `${blockNumber}`
    });
  ...
  public async getBalance(address: string) {
    const balance = (await this.dune.tz.getBalance(address)).toString();
    return balance;
  }

  public async getCode(address: string) {
    const storage = await this.dune.contract.getStorage(address);
    return storage as string;
  }

  public async getAccounts(config: Config) {
    await this.setWallet(config);
    const currentAccount = await this.dune.signer.publicKeyHash();
  ...
  public async getBlockNumber() {
    const { level } = await this.dune.rpc.getBlockHeader();
    return level;
  }
  ...
  public setProvider(provider: Provider) {
    ...
    return this.dune.setProvider({ rpc: currentHost });
  }
  ...
  public async setWallet(config: Config) {
    ...
    if (mnemonic) {
      ...
      try {
        return await this.dune.importKey(
    ...
    if (secretKey) {
      try {
        return await this.dune.importKey(secretKey);
      } catch (error) {
        throw Error(
